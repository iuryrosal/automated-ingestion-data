...
from src.models.vehicle import Vehicle, database

# api_utils functions
from src.utils.api_utils import build_limit_expr
from src.utils.api_utils import build_where_expr
from src.utils.api_utils import get_query_params

# abort functions
from src.utils.api_abort import abort_with_error
from src.utils.api_abort import abort_get_item_if_id_not_exist
from src.utils.api_abort import abort_get_freq_items_by_not_valid_column
...
def get_routes(app):
    ...
    def get_weekly_trips_by_region():
        ...
        try:
            query_str = """
                            SELECT
                              EXTRACT(WEEK FROM datetime)::integer
                              AS week_number,
                              region,
                              COUNT(*) AS freq_trip
                            FROM vehicles_records
    ...
    def get_weekly_avg_trips_by_region():
        '''
            ...
            per week (using datetime column) and region.
        '''
        try:
            query_str = """
                            SELECT region, AVG(freq_trip)::VARCHAR(255)
                            AS freq_avg_weekly_trips
                            FROM
                                (
                                SELECT EXTRACT(WEEK FROM datetime)::INTEGER
                                AS week_number,
                                       region,
                                       COUNT(*) AS freq_trip
                                FROM vehicles_records
