...
def build_where_expr(query_str: str, query_params: str) -> str:
    ...
    if query_params["origin_coord_point_x"]:
        where_expr.append(f"""origin_coord_point_x
                          LIKE
                          '{str(query_params['origin_coord_point_x'])}%%'""")
    if query_params["origin_coord_point_y"]:
        where_expr.append(f"""origin_coord_point_y
                          LIKE
                          '{str(query_params['origin_coord_point_y'])}%%'""")
    if query_params["destination_coord_point_x"]:
        where_expr.append(f"""destination_coord_point_x
                          LIKE
                          '{str(
                            query_params['destination_coord_point_x'])
                           }%%'""")
    if query_params["destination_coord_point_y"]:
        where_expr.append(f"""destination_coord_point_y
                          LIKE
                          '{str(
                            query_params['destination_coord_point_y'])
                           }%%'""")
    if query_params["date"]:
        where_expr.append(f"""datetime::TIMESTAMP::DATE =
                          '{str(query_params['date'])}%%'""")
